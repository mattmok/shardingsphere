...
public class ShardingRule implements BaseRule {
    ...
    private final Map<String, TableRule> tableRulesSets = new LinkedHashMap<>();
    ...
    public ShardingRule(final ShardingRuleConfiguration shardingRuleConfig, final Collection<String> dataSourceNames) {
        ...
        tableRules.forEach(item -> {
            String logicTable = item.getLogicTable().toLowerCase();
            tableRulesSets.put(logicTable, item);
        });
    ...
    public Optional<TableRule> findTableRule(final String logicTableName) {
        String logicTableNameLowerCase = logicTableName.toLowerCase();
        TableRule tableRule = tableRulesSets.get(logicTableNameLowerCase);
        if (tableRule == null && Strings.isNullOrEmpty(shardingDataSourceNames.getDefaultDataSourceName())
                && defaultDatabaseShardingStrategy != null && !isBroadcastTable(logicTableName)) {
            tableRule = new TableRule(shardingDataSourceNames.getDataSourceNames(), logicTableNameLowerCase);
            tableRules.add(tableRule);
            tableRulesSets.put(logicTableNameLowerCase, tableRule);
        }
        return Optional.ofNullable(tableRule);
    }
